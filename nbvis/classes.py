...
class Vis:
    ...
    def __init__(self, *args, js="", silent=True):
        """
        """
        
        # make container for RequireJS sources
        self.sources = ["d3-require"]; # + ['mathBox']
        
        # make container for additional RequireJS modules
        self.modules = modules
        
        # make container for JavaScript code
        self.js = js
        
        # make container for D3 class instances
        self.instances = []
        ...
        for idx, arg in enumerate(args):
            if isinstance(arg, D3):
                self.instances += [arg]
            else:
                name = [v for v in getVariableName(arg) if not v.startswith('_')][0]
                msg = '"{}" is not a D3 class instance'.format(name)
                raise TypeError(msg)
